 ...
 * /dms/api/health, a path the Shell does not have — so a 200 there proves the
 * request genuinely reached THIS app rather than being answered by the Shell.
 *
 * NOTE ON THE ROUTE GUARD: `middleware.ts` does see this request, but
 * `/api/health` matches none of its ROUTE_ROLES prefixes, so it falls through
 * to `NextResponse.next()`. That is the "an unguarded route is one nobody has
 * classified yet, let it through" decision — do not change it to a default-deny
 * without moving this path out of the matcher first, or every probe fails and
